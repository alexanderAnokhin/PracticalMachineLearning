...
# Analysis
## Data Preprocessing
First of all required packages as well as the data are loaded into `R`. The train data consist of `19622` observations and `160` variables including target `classe` variable that should be predicted. The test data consist of `20` cases that should be predicted and submitted. 
```{r}
...
# it is expected that the data are in the same directory as the .Rmd file
...
Apparently, there are `#DIV/0!`'s in some numeric columns and so they were coerced in the wrond way to character vectors. The fix is to replace `#DIV/0!`'s to `NA`'s and coerce them to numeric vectors.
```{r}
...
Another problem of the data is that some predictors have so many `NA` values that makes them useless for a good model. Therefore such variables are excluded from the analysis.
```{r}
...
There are also variables with only unique values, such variables are also excluded from the analysis.
```{r}
...
The last step of data preprocessing procedure is to transfrom character variables to factors. In fact, there are only two variables that are factors, namely `user_name` and `classe`. After all data preprocessing step there are still `19622` observations and only `57` variables left including `classe` variable.
```{r}
...
There are five classes from `A` to `E`, the data a bit more skewed to the label `A`, but still, discrepancy is not that strong. 
```{r}
table(train.in$classe)
```

Since there is only one factor variable `user_name` it is easy to check how it is correlated with `classe` variable. It can be seen from the graph below that there is no bias at all and distribution of `classe` variable is more or less balanced across the `user_name` variable.
```{r}
...
There is some correlation between a sample numerical variables and `classe` variable, but it is hard to conclude something strong based on the scatterplots that underline only univarite interaction between variables.
```{r}
...
In many cases it is easier to work with encoded factor variables, so all the predictors are numerical. Here is used one-hot-encoder to achive the goal. Thus instead of of one factor variable `user-name` six numerical variables are created consisting of only two values `0` and `1`.
```{r}
...
In order to free RAM memory for model fitting unused variables are removed and garbage collectore is explicitly called.
```{r}
rm(
  train.in,
  to.plot.one, 
...
The first model is `Random Forest`. It performs very well with defaults parameters obtaining `99%` of accuracy on test data therefore there is no need for hyper-parameter optimization via validation or crossvalidation.
```{r}
...
The second model is `Gradient Boosting Trees`. The perfromance of that model with default parameters is relatively low compared to `Random Forests`. The accuracy only achives about `82%` which is not acceptable.
```{r results=F}
fit.gbm <- suppressWarnings(
...
The typicall way to increase accuracy is hyper-parameter optimization employing cross-validation at every step to estimate out-of-sample error rate. Here the simpliest grid serach is used supported by `5`-fold cross-validation. 
```{r results=F}
...
predictions.gbm <- predict(fit.gbm.opt, test)
m <- confusionMatrix(data = predictions.gbm, reference = test$classe)
```
...
## Prediction
The agreement ratio of the models accounts for `98.5%` which gives the curtain hopes that out-of-sample error will be at least at that level.
```{r}
predictions <- data.frame("rf" = predictions.rf, "gbm" = predictions.gbm.opt, "xgb" = predictions.xgb)

x <- predictions %>% mutate(agree = rf == gbm & gbm == xgb)
table(x$agree) / length(x$agree)
```

Now it is time to make predictions for the oservations with unknown target variable `classe`. The agreement accuracy is `100%` which is a good indicator. Submitting results to the automatic grader gives accuracy `100%`.
```{r}
...
# Results
The analysis resulted in robust three-model ansemble where the accuracy of every model on unseen dataset is greater than `98%`. The concepts of hyper-parameter-tuning and cross-validation helped to improve accuracy of `Gradient Boosting Classification Trees` from only `82%` to about `100%` accuracy.
